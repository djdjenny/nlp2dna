...
class DNAOverlappingCollator:
    ...
    def __call__(self, examples):
        ...
        for b in range(batch_size):
            ...
            num_spans = max(1, num_to_mask // (2 * self.k - 1))
            max_start = num_tokens - 2 * self.k + 2

            if max_start <= 0:
                continue

            span_starts = random.sample(range(1, max_start + 1), k=min(num_spans, max_start))

            for start in span_starts:
                masked_indices[b, start : start + (2 * self.k - 1)] = True
...
def build_components(mode, train_csv, val_csv, mlm_probability):
    if mode == "bpe":
        ...
        collator = DNAOverlappingCollator(
            tokenizer,
            mlm_probability=mlm_probability,
        )
...
def create_parser():
    ...
    parser.add_argument(
        "--mode",
        required=True,
        choices=["bpe", "3mer", "6mer", "nucleotide"],
